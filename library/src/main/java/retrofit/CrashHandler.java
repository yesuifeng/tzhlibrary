    ...
    private CrashHandler() {
        ...
        public void uncaughtException(Thread t, Throwable e) {
            if (!handleException(e)) {
                ...
                Intent intent = null;
                try {
                    intent = new Intent(mContext, Class.forName("com.guangan.merchant.WelcomeActivity"));
        ...
        //        cause = e.getCause();
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                if (!file.exists()) file.createNewFile();
                    ...
                    mEntity.setUrl(file.getAbsolutePath());
                    mEntity.setType(LOG);
                    List<CustomHttpEntity> dataList = HttpSharedUtils.getDataList(LOG,CustomHttpEntity.class);
                    dataList.add(mEntity);
                    HttpSharedUtils.setDataList(LOG,dataList);
                } catch (Exception e1) {
                    e1.printStackTrace();
                } finally {
                    if (fos != null) {
